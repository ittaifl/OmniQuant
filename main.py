...
def log_magnitudes(args, logger):
    import matplotlib.pyplot as plt
    import os

    for layer_num, nest_dict in get_magnitudes().items():
        
        # Creating subplots with multiple histograms
        fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(36, 8))
        i = 0
        ...
        plt.savefig(os.path.join(args.output_dir, f"masks_plot_layer_{layer_num}"))
...
def main():
    ...
    parser.add_argument("--ngumbel_samples", type=int, default=1024, help="number of gumble data samples.")
    parser.add_argument("--gumbel_batch_size", type=int, default=10, help="size of batch of gumble samples")
    parser.add_argument("--prune_method", type=str, default='pman', choices=prune_methods, help='which prune method to combine with quantization')
    parser.add_argument("--var_lr", type=float, default=1e-3)
    parser.add_argument("--var_train_clip", default=False, action="store_true", help="Train variational with clipping")
